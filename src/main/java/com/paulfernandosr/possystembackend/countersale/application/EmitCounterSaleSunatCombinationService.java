...
public class EmitCounterSaleSunatCombinationService implements EmitCounterSaleSunatCombinationUseCase {
    ...
    public CounterSaleSunatCombinationEmitResponse emit(Long anchorCounterSaleId,
        ...
        SaleV2SunatEmissionResponse emission = emitSaleV2ToSunatUseCase.emit(generatedSaleId);

        if (emission == null) {
            String reason = "SUNAT/PHP no devolvió respuesta para la venta diaria.";
            updateSaleSunat(generatedSaleId, "ERROR_COMUNICACION", null, reason, null, null, null, null, LocalDateTime.now());
            markComboStatus(comboId, "ERROR_COMUNICACION", reason);
            return buildCombinationResponse(result, comboId, generatedSaleId, nextNumber, null, false, null);
        }
        ...
        if (!"ACEPTADO".equalsIgnoreCase(emission.getSunatStatus())) {
            ...
            return buildCombinationResponse(result, comboId, generatedSaleId, nextNumber, emission, false, null);
        }

        LocalDateTime emittedAt = emission.getEmittedAt() != null
                ? emission.getEmittedAt()
                : LocalDateTime.now();
        ...
        return buildCombinationResponse(result, comboId, generatedSaleId, nextNumber, emission, true, emittedAt);
    }

    private CounterSaleSunatCombinationEmitResponse buildCombinationResponse(
            CounterSaleSunatCombinationComposer.ComposedResult result,
            Long comboId,
            Long generatedSaleId,
            Long number,
            SaleV2SunatEmissionResponse emission,
            boolean accepted,
            LocalDateTime associatedAt
    ) {
        String sunatStatus = emission != null ? emission.getSunatStatus() : "ERROR_COMUNICACION";
        String sunatCode = emission != null ? emission.getSunatCode() : null;
        String sunatDescription = emission != null
                ? emission.getSunatDescription()
                : "SUNAT/PHP no devolvió respuesta para la venta diaria.";
        String hashCode = emission != null ? emission.getHashCode() : null;
        String xmlPath = emission != null ? emission.getXmlPath() : null;
        String cdrPath = emission != null ? emission.getCdrPath() : null;
        String pdfPath = emission != null ? emission.getPdfPath() : null;
        LocalDateTime emittedAt = emission != null ? emission.getEmittedAt() : null;
        ...
        return CounterSaleSunatCombinationEmitResponse.builder()
                ...
                .emission(CounterSaleSunatEmissionInfoResponse.builder()
                        ...
                        .pdfPath(pdfPath)
                        .emittedAt(emittedAt)
                        .accepted("ACEPTADO".equalsIgnoreCase(sunatStatus))
                        .rejected("RECHAZADO".equalsIgnoreCase(sunatStatus))
                        .communicationError("ERROR_COMUNICACION".equalsIgnoreCase(sunatStatus))
                        .retryable("ERROR_COMUNICACION".equalsIgnoreCase(sunatStatus) || "ERROR".equalsIgnoreCase(sunatStatus))
                        .build())
                .linkedCounterSales(result.getSelectedCounterSales().stream().map(row -> CounterSaleSunatCombinationCounterSaleResponse.builder()
                        ...
                        .associatedToSunat(accepted)
                        .associatedSaleId(accepted ? generatedSaleId : null)
                        .associatedDocType(accepted ? "BOLETA" : null)
    ...
    private void markComboStatus(Long comboId, String status, String message) {
        String sql = """
                UPDATE counter_sale_sunat_combo
                   SET combo_status = ?,
                       error_message = ?,
                 ...
                 WHERE id = ?
                """;
        jdbcClient.sql(sql).params(status, truncate(message, 4000), comboId).update();
    }
