...
class CounterSaleSunatCombinationComposer {
    ...
    private void ensureEligible(CounterSaleDetailResponse detail, List<String> errors) {
        ...
        if ("ACEPTADO".equalsIgnoreCase(blank(detail.getLinkedSunatStatus())) ||
                "ACEPTADO".equalsIgnoreCase(blank(detail.getLinkedProcessStatus()))) {
            errors.add("El counter-sale ya fue aceptado por SUNAT. counterSaleId=" + detail.getCounterSaleId());
        }
    }
